...
def kama_market_regime(df, col, n, m):
    ...
    df_copy["kama_diff"] = (df_copy[f"kama_{m}"] - df_copy[f"kama_{n}"]) / df_copy[f"kama_{n}"]
    df_copy["kama_trend"] = -1
    df_copy.loc[0<df_copy["kama_diff"], "kama_trend"] = 1
    
    return df_copy

def rolling_correlation(df: pd.DataFrame, window_size: int = 20, min_periods: int = 5, lag: int = 1,
                        close_col1: str = 'close', close_col2: str = 'close') -> pd.Series:
    """
...
def regime_shifts( df: pd.DataFrame, close_col: str = 'close', threshold: float = 1.5) -> pd.Series:
    """
...
def close_percentage_in_range(series, low, high, start_percentage, end_percentage):
    """
    Calculates the percentage of closing prices within a specified range, 
    defined as a proportion of the range between `low` and `high`.
    ...
    low : float
        The minimum value of the range (e.g., the lowest price in the interval).
    high : float
        The maximum value of the range (e.g., the highest price in the interval).
    start_percentage : float
        The starting percentage of the range (e.g., 0.25 for 25%).
    end_percentage : float
        The ending percentage of the range (e.g., 0.75 for 75%).

    Returns:
    --------
    float
        The percentage of closing prices within the specified range. Returns 0 
        if the total number of data points is zero.
    """
...
def apply_close_percentage_in_range(df, start_percentage, end_percentage):
    """
    Aggregates the percentage of closing prices within a specified range for a 
    given DataFrame containing 'low', 'high', and 'close' columns.

    Parameters:
    -----------
    df : pandas.DataFrame
        Input DataFrame containing 'low', 'high', and 'close' columns.
    start_percentage : float
        The starting percentage of the range (e.g., 0.25 for 25%).
    end_percentage : float
        The ending percentage of the range (e.g., 0.75 for 75%).
    ...
    --------
    float
        The percentage of closing prices within the specified range.
    """
    low = df['low'].min()
    high = df['high'].max()
...
def linear_regression_slope(series):
    """
    Calculates the slope of a linear regression line fitted to a time series.

    Parameters:
    -----------
    series : pandas.Series
        The input time series for which the slope is to be calculated.
...
def apply_linear_regression_slope(df):
    """
    Applies linear regression to the closing prices of a given DataFrame and 
    calculates the slope of the regression line.

    Parameters:
    -----------
    df : pandas.DataFrame
        Input DataFrame containing the 'close' column.

    Returns:
    --------
    float
        The slope of the linear regression line for the closing prices. Returns 
...
def apply_linear_regression_slope_last_25(df):
    """
    Applies linear regression to the last 25% of the closing prices in a given 
    DataFrame and calculates the slope of the regression line.
    ...
    --------
    float
        The slope of the linear regression line for the last 25% of closing 
        prices. Returns NaN if an exception occurs.
    """
    close_prices = df['close']
...
def linear_regression_slope_market_trend(series):
    """
    Calculates the slope of the linear regression line for a given time series.

    Parameters:
    -----------
    series : pandas.Series or numpy.ndarray
        The input time series for which the linear regression slope will be 
        calculated. Each value in the series represents an observation.
